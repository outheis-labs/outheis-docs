# Implementierung

*Der aktuelle Stand — was gebaut ist und wie man es benutzt.*

## Dokumente

### [Architektur](architecture.md)

Aktuelle Systemarchitektur: dispatcher, agents, Wissensspeicher, Datei-Layout, geplante Aufgaben.

### [Konfiguration](config.md)

Vollständige `config.json`-Referenz: Anbieter, Modelle, agents, Zeitpläne.

### [Memory & Rules](memory.md)

Wie outheis sich erinnert: Memory-Typen, explizite Marker, Muster-Extraktion, Rules-Entstehung.

### [Skills](skills.md)

Wie agents wissen, was zu tun ist: System-Skills, gelernte Skills, der Unterschied zwischen Skills und Rules.

### [Agenda](agenda.md)

Zeitmanagement durch Daily.md, Inbox.md, Exchange.md. Stündliche Überprüfung, manuelle Aktualisierung, Hash-basierte Optimierung.

### [Migration](migration.md)

Bestehendes Wissen in outheis bringen: vault/Migration-Workflow, Genehmigung über Checkboxen.

### [Vault](vault.md)

Wie du dein Wissen strukturierst: Datei-Layout, Tag-Systeme, das facettierte Namensraum-Muster, Daily.md-Vorlage.

### [CLI-Anleitung](guide.md)

Kommandozeilenschnittstelle: Installation, Befehle, Nutzungsbeispiele.

### [Signal](signal.md)

Signal Messenger Transport: Leser-Thread-Architektur, Markdown-Entfernung, Autorisierung, Sprachtranskription.

### [Annotations-Feedback-Schleife](annotation-feedback.md)

Wie `>`-Annotationen in Agenda.md in den Memory-Stack geroutet werden sollen — die drei Annotationstypen, die fehlende Feedback-Schleife und die geplante cato+rumi-Implementierung.
