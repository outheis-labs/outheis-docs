...
| **qwen3:14b** | 14B | 6/9 | 2 | 1 | halluziniert beim Leer-Ergebnis-Test; ~421s gesamt |
...
| qwen2.5-coder:14b | 14B | 1/9 | 0 | 8 | Keine Tool-Calls — nicht geeignet |
...
`qwen3:14b` erreicht ebenfalls 6/9 — gleichauf mit den besten Modellen — benötigt dafür aber ~421s gesamt (~47s pro Szenario). Bei dieser Geschwindigkeit wird ein mehrstufiger Agentenaustausch für den interaktiven Einsatz unpraktisch. Zusätzlich halluziniert es beim Leer-Ergebnis-Szenario. Auf M4/24 GB nicht für den Alltagseinsatz empfohlen.

`qwen2.5-coder:14b` erreicht 1/9: Es ruft in acht von neun Szenarien keine Tools auf und gibt stattdessen Freitext aus. Das einzige erfolgreiche Szenario ist eine Identitätsabfrage ohne Tool-Bedarf. Nicht geeignet für generische Agenten.
...
| qwen2.5-coder:14b | 14B | 5/10 | 2 | 3 | Extraktionsphase scheitert; Konsolidierung/Promotion solide; ~116s gesamt |
| qwen3:14b | 14B | 4/10 | 3 | 3 | Extraktionsphase scheitert; Halluzination bei extract_empty; ~444s gesamt |
...
`qwen2.5-coder:14b` erreicht 5/10 mit einem auffälligen Muster: Konsolidierung und Promotion funktionieren gut (3/3 korrekt oder partiell), aber die Extraktion scheitert vollständig — 0 von 3 Einträgen korrekt klassifiziert, und Halluzination beim Leer-Extraktions-Szenario. Da Extraktion die erste Phase ist, die rumi ausführt, korrumpieren Fehler hier alle nachgelagerten Phasen. Mit ~116s Gesamtlaufzeit das schnellste getestete Modell für rumi — aber wegen der Extraktionsfehler für den produktiven Einsatz nicht geeignet.

`qwen3:14b` erreicht 4/10 mit denselben Extraktionsfehlern wie qwen2.5-coder:14b, zusätzlich schwächerer Konsolidierung (findet nur 1 von 2 Duplikaten). Mit ~444s Gesamtlaufzeit das langsamste getestete Modell. Nicht geeignet.
