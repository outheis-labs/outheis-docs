...
Die meisten aktuellen agent-Frameworks erfinden diese Lösungen mangelhaft nach und führen zu fragilen Architekturen, die unter Komplexität scheitern.

## Die Erkenntnis

Ein Betriebssystem ist im Grunde ein System zur Koordination unabhängiger Prozesse, die:
...
3. Graceful scheitern müssen, ohne das gesamte System zum Absturz zu bringen
4. Von einfachen zu komplexen Workloads skalieren müssen

Ein Multi-agent-KI-System hat dieselben Anforderungen. Der Unterschied besteht darin, dass statt Prozessen, die Speicher und Dateien manipulieren, agents Kontext manipulieren und Antworten generieren.
...
Wenn mehrere agents auf gemeinsamen Daten operieren, führen naive Implementierungen zu Race-Conditions — nicht im traditionellen Sinne von Speicherkorrumpierung, sondern im Sinne von konfligierenden Aktionen, doppelter Arbeit oder verlorenem Kontext.

OS-Lösungen: Message-Passing, Ownership-Semantik, Locks.

### 2. Klare Grenzen

Prozesse haben isolierte Adressräume. Agents sollten isolierte Domains haben. Wenn ein Daten-agent alle Datenoperationen besitzt, können andere agents diese Domain nicht korrumpieren — sie müssen durch eine definierte Schnittstelle anfragen.

### 3. Fehler-Isolation

Ein abstürzender Prozess sollte das System nicht zum Absturz bringen. Ebenso sollte ein halluzinierter oder fehlschlagender agent nicht das gesamte Gespräch oder die Wissensbasis korrumpieren.

### 4. Kompositionsfähigkeit

Unix-Philosophie: kleine Werkzeuge, die eine Sache gut machen, durch Pipes kombiniert. agent-Philosophie: spezialisierte agents mit klaren Verantwortlichkeiten, durch Message-Passing kombiniert.
...
Übersetzung: Jeder agent kann seine eigene Message-Queue haben. Kein zentraler dispatcher-Engpass. Ownership bestimmt, wer was modifizieren kann.
...
Übersetzung: Agents haben explizite Capabilities. Ein Pattern-agent kann Insights schreiben; andere können nur lesen. Der dispatcher ist minimal — nur Routing.
...
- **Privilegtrennung**: Aufgeteilt in privilegierten Elternteil (hält Ressourcen) und unprivilegierten Kindprozess (erledigt die Arbeit)
- **Sicher per Standard**: Alles aus, bis explizit aktiviert

Übersetzung: Agents erklären ihre Capabilities beim Start — keine impliziten Berechtigungen. Jeder agent sieht nur seine relevanten Pfade (vault, insights). Der dispatcher hält Zugang zur Queue; agents laufen mit minimalem Zugang.

### macOS / Grand Central Dispatch

- **Dispatch-Queues**: Arbeit in Queues einreichen statt Threads zu verwalten
...
5. **Capability-basierter Zugang**
   Agents haben explizite Berechtigungen. Ein relay-agent kann auf externe Schnittstellen zugreifen; ein action-agent nicht.
