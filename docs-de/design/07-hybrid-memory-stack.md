...
Zwei Versagensmuster definieren den Designraum.

**Reiner Code** kann nicht lernen. Schlüsselwortlisten, Regex-Muster, fest kodierte Klassifikatoren, Icon-Mappings, Format-Detektoren — sie kodieren das heutige Verständnis und sperren es gleichzeitig ein. Wenn du etwas anders formulierst, in einer anderen Sprache schreibst oder ein unerwartetes Format verwendest, schlägt die Heuristik still fehl. Weitere Verzweigungen hinzuzufügen, um Randfälle zu behandeln, ist kein Lernen — es ist das Ansammeln technischer Schulden, die schneller wachsen, als das System sich verbessert.

**Reines LLM** skaliert nicht. Jede Entscheidung an ein Sprachmodell zu delegieren — einschließlich Entscheidungen, die deterministisch getroffen werden könnten — ist teuer, langsam, unvorhersehbar und undurchsichtig. Keine stabilen Garantien: Dieselbe Datei könnte bei zwei aufeinanderfolgenden Läufen unterschiedlich klassifiziert werden.
...
- Nachrichtenaustausch zwischen agents (Warteschlange, dispatcher, Routing)
...
Diese Dinge ändern sich nicht, weil du etwas anders formuliert hast. Ihre Korrektheit ist durch Tests überprüfbar — unabhängig von Sprache, Kontext oder Präferenz.
...
- Formatierung gemäß beobachteten Präferenzen
- Strukturiertes Wissen aus Freitext extrahieren (Dateien, Nachrichten, Notizen)
- Konflikte, Prioritäten und Relevanz beurteilen

Kein statischer Algorithmus erkennt subtile Muster in natürlicher Sprache über Sitzungen und Sprachen hinweg. Ein Sprachmodell kann es — und wird besser darin, je mehr Skills und Memory das System destilliert hat.
...
Die Agentenschleife ist strukturell einheitlich über alle agents:
...
Das Ziel: Ein System, das mit der Zeit progressiv weniger Kontext benötigt — weil Skills bereits erfasst haben, was gewusst werden muss, und Code das behandelt, was sich nie ändern wird.
