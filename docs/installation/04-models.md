...
| **qwen3:14b** | 14B | 6/9 | 2 | 1 | hallucinates on empty-results test; ~421s total |
...
| qwen2.5-coder:14b | 14B | 1/9 | 0 | 8 | No tool calls — not suitable |
...
`qwen3:14b` also scores 6/9 — matching the top tier — but at ~421s total (~47s per scenario), it is by far the slowest model at this score level. At that throughput, a multi-agent exchange becomes impractical for interactive use. It also hallucinates on the empty-results scenario. Not recommended for daily use on M4/24 GB.

`qwen2.5-coder:14b` scores 1/9: it calls no tools for the first eight scenarios, producing plain text answers instead. The single pass is a no-tool-needed identity query. Not suitable for generic agents.
...
| qwen2.5-coder:14b | 14B | 5/10 | 2 | 3 | extract phase fails; consolidate/promote solid; ~116s total |
| qwen3:14b | 14B | 4/10 | 3 | 3 | extract phase fails; hallucination on extract_empty; ~444s total |
...
`qwen2.5-coder:14b` scores 5/10 with a notable split: consolidation and promotion phases work well (3/3 correct or partial), but extraction fails entirely — it classifies 0 of 3 entries correctly and hallucinates on the empty-extraction scenario. Since extraction is the first phase rumi runs, failures here corrupt all downstream phases. At ~116s total it is the fastest model tested for rumi, but extraction failures make it unsuitable for production use.

`qwen3:14b` scores 4/10 with the same extraction failures as qwen2.5-coder:14b plus weaker consolidation (finds only 1 of 2 duplicates). At ~444s total it is the slowest model tested. Not suitable.
