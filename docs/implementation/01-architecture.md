...
└──────────────────────┬──────────────────────────────┘
    ...
    ▼                  ▼              ▼              ▼
┌───────┐    ┌───────┐    ┌───────┐    ┌───────┐  ┌───────┐
│  ou   │    │ zeno  │    │ cato  │    │ rumi  │  │ alan  │
│ relay │    │ data  │    │agenda │    │pattern│  │ code  │
└───┬───┘    └───┬───┘    └───┬───┘    └───┬───┘  └───┬───┘
    │            │             │             │          │
    ▼            ▼             ▼             ▼          ▼
┌─────────┐  ┌───────┐   ┌────────┐   ┌────────┐  ┌────────┐
│   LLM   │  │ Vault │   │ Agenda │   │ Memory │  │ Source │
│ (Haiku) │  │       │   │  dir   │   │        │  │  code  │
└─────────┘  └───────┘   └────────┘   └────────┘  └────────┘
```
...
Explicit mentions (@zeno, @cato, @hiro, @rumi, @alan) still work for direct delegation.
...
**`pattern_infer` vs `memory_migrate`:** These are distinct operations. `pattern_infer` runs nightly and extracts/consolidates patterns from recent conversations into memory, rules, and skills. `memory_migrate` is a separate task (disabled by default) that processes structured seed files placed in `vault/Migration/` — typically used to import existing notes or contact data on initial setup. See [Migration](migration.md).
...
|------|--------------|---------|
| `pattern_infer` | 04:00 | Extract memories, consolidate, promote rules |
